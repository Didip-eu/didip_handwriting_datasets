...
class Alphabet:
    ...
    def __init__( self, alpha_str: str='', alpha_tsv: str=''):
        if alpha_str:
            self._code_2_utf = self.from_string( alpha_str )
        elif alpha_tsv:
            self._code_2_utf = self.from_tsv( alpha_tsv )  
    ...
    @classmethod
    def prototype(cls, file_paths: List[Path]) -> str:
        charset = set()
        ...
        return "\n".join( ["0\t\u2205"] + [f"{c}\t{s}" for (c,s) in enumerate(sorted(charset), start = 1) ] )
